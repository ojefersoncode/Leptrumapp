...
>(({ children, className, ...props }, ref) => {
  return (
    ...
    >
      <span
        className="relative block size-full text-xs p-1.5 uppercase tracking-wide text-white"
        style={{
          maskImage:
      ...
      <span
        ...
        className="absolute inset-0 z-10 block border rounded-md  bg-[linear-gradient(-75deg,hsl(var(--primary)/10%)_calc(var(--x)+20%),hsl(var(--primary)/50%)_calc(var(--x)+25%),hsl(var(--primary)/10%)_calc(var(--x)+100%))] p-px"
      ></span>
    </motion.button>
  );
});
